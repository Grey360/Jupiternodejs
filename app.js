...
io.sockets.on('connection', function (socket, pseudo, team) {
  ...
  socket.on('nouveau_client', function(pseudo, score, team) {
   ...
   socket.emit('clic_Callisto', team1_score,team2_score);
  ...
  socket.on('clic_Callisto', function (team) {
	...
	if(team == "TERRANS")
		team1_score +=socket.multi;
	else
		team2_score +=socket.multi;
		
	socket.score += socket.multi;
	console.log("TERRANS", team1_score);
	console.log("ZERGS", team2_score);
	message = "test";

   socket.broadcast.emit('clic_Callisto', team1_score,team2_score);
   socket.emit('clic_Callisto', team1_score,team2_score);
